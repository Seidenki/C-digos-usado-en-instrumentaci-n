...
/***Variables internas del LoRa***/
int inicia=0;
int bit_confirmacion=1;
int confirma_mensaje2=0;
...
void loop() {
  ...
  if (bit_confirmacion==1){          //Si debe encender el LoRa
    
    if(inicia==0){LoRa_setup();inicia=1;} //Esta linea enciende el Lora y solo se ejecuta una vez

    if ((io.status() < AIO_CONNECTED)) { //si tiene conexión a internet significa que no es esta estacion la que perdió la conexión
      //Por tanto se convierte en el Master
      Master_loop();

    }else{                                //Por el contrario es quien perdió la conexión, luego no se va a poder conectar al Adafruit
      //Por tanto se convierte en el Slave
      confirma_mensaje2=1; //Bit que manda el Slave al master para decirle que ya envió todo el mensaje
      Slave_loop(&Temperatura_ex, &Presion_ex, &Humedad_ex, &dustDensity_ex, &CO2_ex, &VOC_ex, confirma_mensaje2);
...
void LeerSensorUV() {
  ...
  float uvIntensity = mapfloat(uvLevel, 0, 1023, 0.0, 15.0);
